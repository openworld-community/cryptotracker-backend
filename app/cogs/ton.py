...
async def process_transactions_from_api(transactions):
    ...
    logging.info(f"""Processing {len(transactions)-len(existing_transactions_hashes)} new transactions""")
    ...
    for transaction in transactions:
        ...
        db.add_processed_transaction(
            parsed["currency"], parsed["amount"], parsed["transaction_hash"],
            parsed["timestamp"], parsed["from"], parsed["to"])
...
@router.post(
    ...
    status_code=201,
    response_description="Transaction added to pending queue",
...
async def add_pending_transaction(
    ...
    db.add_pending_transaction(CURRENCY, transaction_request.amount, transaction_request.ttl)
